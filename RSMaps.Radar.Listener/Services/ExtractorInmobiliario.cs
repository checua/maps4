...
public static class ExtractorInmobiliario
{
        ...
        string messageId)
    {
        ...
        (solicitud.RecamarasMin, solicitud.RecamarasMax) =
            ExtraerRangoEntero(
                normalizado,
                @"(\d+)\s*(?:a|-|hasta)\s*(\d+)\s*(?:recamaras?|habitaciones?|cuartos?)",
                @"(\d+)\s*(?:recamaras?|habitaciones?|cuartos?)");

        (solicitud.BanosMin, solicitud.BanosMax) =
            ExtraerRangoEntero(
                normalizado,
                @"(\d+)\s*(?:a|-|hasta)\s*(\d+)\s*banos?",
                @"(\d+)\s*banos?");

        (solicitud.PrecioMinimo, solicitud.PrecioMaximo) =
            ExtraerRangoPrecio(normalizado, solicitud.Operacion);

        if (solicitud.Operacion is null &&
            solicitud.ModalidadesPago.Any(x =>
                x is "Infonavit" or "Fovissste" or "Crédito hipotecario" or "Crédito bancario" or "Contado"))
        {
    ...
    private static (decimal? Min, decimal? Max) ExtraerRangoPrecio(
        string texto,
        string? operacion)
    {
        ...
        // Acepta: 2,500,000 | 2.500.000 | 800 000 | 2500000 | 25
        // pero NO absorbe el siguiente dato de la frase, por ejemplo
        // "$2,500,000, 3 recámaras".
        const string numero = @"(?:\d{1,3}(?:[ ,.]+\d{3})+|\d+)";
        ...
        if (maximo.Success)
            return (null, ParsePrecio(maximo.Groups[1].Value, operacion));
        ...
        if (millones.Success)
        {
            ...
            if (numeroMillones.HasValue)
                return (null, numeroMillones.Value < 100
                    ? numeroMillones.Value * 1_000_000
                    : numeroMillones.Value);
        }
        ...
        if (montoSimple.Success)
            return (null, ParsePrecio(montoSimple.Groups[1].Value, operacion));
    ...
    private static decimal? ExtraerSuperficieMinima(string texto)
    {
        ...
        return match.Success
            ? ParseDecimalFlexible(match.Groups[1].Value)
            : null;
    }
    ...
    private static List<string> ExtraerZonas(string texto)
    {
        ...
        var patrones = new[]
        {
            @"\bubicacion\s+ideal\s*:\s*([^\n\.]+)",
            @"\b(?:en\s+renta|en\s+venta)\s+en\s+([^\n\.]+)",
            @"\b(?:ubicad[ao]s?\s+en|ubicad[ao]s?\s+por\s+rumbos\s+de|rumbos?\s+de|rumbo\s+a|rumbo\s+al|por\s+el\s+lado\s+de|por\s+la\s+zona\s+de|por\s+zona\s+de|zona\s+de|exclusivamente\s+en|cerca\s+de)\s+([^\n\.]+)",
        ...
        foreach (var patron in patrones)
        {
            foreach (Match match in Regex.Matches(texto, patron, RegexOptions.IgnoreCase))
            {
                ...
                foreach (var parte in Regex.Split(
                             bloque,
                             @"\s*(?:,|\bo\b|\by\b|/|;)\s*",
                             RegexOptions.IgnoreCase))
                {
    ...
    private static string LimpiarZona(string valor)
    {
        return Regex.Replace(valor, @"\s+", " ")
            ...
            .Replace("alrededores por", "", StringComparison.OrdinalIgnoreCase)
            .Trim(' ', ',', '-', ':');
    }
    ...
    private static bool? ExtraerUnaPlanta(string texto)
    {
        return Regex.IsMatch(
            texto,
            @"\b(1|una|un)\s+planta\b|\bde\s+un\s+piso\b")
            ? true
            : null;
    }
    ...
    private static int? ExtraerCochera(string texto)
    {
        var match = Regex.Match(
            texto,
            @"cochera.*?(\d+)\s*(?:auto|autos|vehiculo|vehiculos)",
            RegexOptions.IgnoreCase);

        if (match.Success && int.TryParse(match.Groups[1].Value, out var autos))
            return autos;

        return texto.Contains("cochera") ? 1 : null;
    }
    ...
    private static List<string> ExtraerModalidadesPago(string texto)
    {
        ...
        {
            if (texto.Contains(opcion.Key) &&
                !modalidades.Contains(opcion.Value, StringComparer.OrdinalIgnoreCase))
            {
