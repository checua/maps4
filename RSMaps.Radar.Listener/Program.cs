...
Console.WriteLine("Después del envío Radar volverá al chat origen e intentará marcar Propiedades como no leído desde la lista.");
    ...
    try
    {
        foreach (var chat in RadarSettings.ChatsMonitoreados)
        {
            ...
            foreach (var solicitud in resultado.Solicitudes)
            {
                ...
                {
                    Console.WriteLine($"  📤 Alerta enviada a '{AlertSettings.ChatDestino}' y chat marcado como no leído desde la lista.");
                }
                else
                {
                    Console.WriteLine($"  📤 Alerta enviada a '{AlertSettings.ChatDestino}'. ⚠ No pude marcar el chat como no leído desde la lista.");
                }
            }
        }
    }
...
static async Task<ILocator?> ObtenerElementoChatVisible(IPage page, string nombreChat)
{
    var titulos = page.Locator("[data-testid='cell-frame-title']");
    ...
    for (var i = 0; i < total; i++)
    {
        ...
        try
        {
            ...
            var fila = titulo.Locator("xpath=ancestor::*[@role='row' or @role='listitem' or @tabindex='-1' or @tabindex='0'][1]");
    ...
    foreach (var candidato in CandidatosTitulo(nombreChat))
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            try
            {
                ...
                var fila = item.Locator("xpath=ancestor::*[@role='row' or @role='listitem' or @tabindex='-1' or @tabindex='0'][1]");
                return await fila.CountAsync() > 0 ? fila.First : item;
            }
            catch { }
        }
    }
...
static async Task<(bool Enviada, bool MarcadoNoLeido, string Detalle)> EnviarAlerta(IPage page, SolicitudInmobiliaria s)
{
    try
    {
        ...
        if (!regresoOrigen)
            return (true, false, "enviado; no se pudo volver al chat origen");

        await Task.Delay(300);
        var marcado = await MarcarChatComoNoLeidoDesdeLista(page, AlertSettings.ChatDestino);
        return (true, marcado, marcado ? "ok" : "enviado; no se pudo marcar no leído desde lista");
    }
    catch (Exception ex)
    {
...
static async Task<bool> MarcarChatComoNoLeidoDesdeLista(IPage page, string nombreChat)
{
    try
    {
        await LimpiarBusqueda(page);

        var elemento = await ObtenerElementoChatVisible(page, nombreChat);
        if (elemento is null)
        {
            var input = await ObtenerInputBusqueda(page);
            if (input is null)
            ...
            elemento = await ObtenerElementoChatVisible(page, nombreChat);
        }
        ...
        await elemento.ClickAsync(new LocatorClickOptions
        {
