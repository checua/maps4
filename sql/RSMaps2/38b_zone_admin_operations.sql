/* ============================================================
   RSMaps 2.0 - Paso 38B
   OPERACIONES ADMINISTRATIVAS DE ZONAS

   Objetivo:
   - Crear/editar una zona privada de la cuenta autenticada.
   - Guardar un poligono editable desde Google Maps.
   - Obtener una zona para edicion.
   - Recalcular automaticamente los inmuebles de la cuenta al guardar.

   Nota:
   - La UI inicial maneja un poligono principal por zona.
   - La tabla RSMAPS_ZonaPoligono ya soporta varios poligonos para una fase posterior.
   ============================================================ */
...
IF DB_NAME() <> 'mapsMarkers'
    THROW 53850, 'Este script debe ejecutarse en la base mapsMarkers.', 1;

IF OBJECT_ID(N'dbo.RSMAPS_Zona', N'U') IS NULL
    THROW 53851, 'No existe dbo.RSMAPS_Zona. Ejecutar primero Paso 38A.', 1;

IF OBJECT_ID(N'dbo.RSMAPS_ZonaPoligono', N'U') IS NULL
    THROW 53852, 'No existe dbo.RSMAPS_ZonaPoligono. Ejecutar primero Paso 38A.', 1;
GO
...
BEGIN
    ...
    /* MVP 38B: sustituye el poligono principal de la zona. */
    ...
    SELECT
        ...
        @codigo AS Codigo,
        @nombre AS Nombre,
        'OK - ZONA GUARDADA Y CUENTA RECLASIFICADA' AS Estado;
END;
GO

/* Prueba de instalacion de procedimientos, sin crear datos permanentes. */
SELECT
    ...
    'OK - PASO 38B SQL INSTALADO' AS Estado;
