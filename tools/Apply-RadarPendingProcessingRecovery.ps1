...
function Find-CSharpBlockEnd {
    ...
    if ($braceStart -lt 0) {
        throw 'Could not locate opening brace for C# block.'
    }
    ...
    throw 'Could not locate closing brace for C# block.'
}
...
foreach ($newPath in @($modelsPath, $interfacePath, $repositoryPath, $controllerPath, $clientPath)) {
    if (Test-Path -LiteralPath $newPath) {
        throw "Target file already exists; helper was probably already applied: $newPath"
    }
}
...
public sealed class RadarProcessingRecoveryController : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (agent is null)
            return Unauthorized(new { mensaje = "RADAR Agent no valido, revocado o sin cuenta activa." });
...
$webProgram = Replace-Once -Text $webProgram -Old $diAnchor -New $diReplacement -Description 'web DI registration for pending processing repository'
...
$listenerProgram = Replace-Once `
    -Text $listenerProgram `
    -Old $deliveryRecoveryAnchor `
    -New ($processingRecoveryCall + $deliveryRecoveryAnchor) `
    -Description 'processing recovery call before durable delivery recovery'

$foreachMarker = '            foreach (var messageId in resultado.DemandasInterpretadas)'
$foreachStart = $listenerProgram.IndexOf($foreachMarker, [System.StringComparison]::Ordinal)
if ($foreachStart -lt 0) {
    throw 'Could not locate ordinary downstream foreach block.'
}

$foreachEnd = Find-CSharpBlockEnd -Text $listenerProgram -StartIndex $foreachStart
...
$sharedBlock = $oldDownstreamBlock.Substring($oldDownstreamBlock.IndexOf('{') + 1)
$sharedBlock = $sharedBlock.Substring(0, $sharedBlock.LastIndexOf('}'))
...
Write-Host 'RADAR pending central processing recovery applied.'
Write-Host 'Created:'
...
Write-Host '  RSMaps.Radar.Listener/Services/RadarProcessingRecoveryClient.cs'
Write-Host 'Modified:'
Write-Host '  Program.cs'
Write-Host '  RSMaps.Radar.Listener/Program.cs'
Write-Host ''
Write-Host 'Next: build maps4.csproj and RSMaps.Radar.Listener.csproj before running A or B.'
