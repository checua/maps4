        ...
        private static async Task<List<Inmueble>> LeerInmueblesAsync(SqlCommand cmd)
        {
            ...
            while (await dr.ReadAsync())
            {
                lista.Add(new Inmueble
                {
                    ...
                    Terreno = dr["terreno"] == DBNull.Value ? 0f : Convert.ToSingle(dr["terreno"]),
                    Construccion = dr["construccion"] == DBNull.Value ? 0f : Convert.ToSingle(dr["construccion"]),
                    Precio = dr["precio"] == DBNull.Value ? 0f : Convert.ToSingle(dr["precio"]),
